// Enable some mandatory OpenGL extensions
#extension GL_EXT_texture_array : enable

// Import jME GLSL tools
#import "terra/shader/ParallaxArray.glsllib"
...
void main(){
    ...
    // ***********************
      ...
      vec4 specularColor = vec4(1.0);
       ...
       #ifdef COLORRAMP
            diffuseColor.rgb  *= texture2DArray(m_ColorRamp, vec3(light.x, 0.0, newTexCoord.z)).rgb;
            specularColor.rgb *= texture2DArray(m_ColorRamp, vec3(light.y, 0.0, newTexCoord.z)).rgb;
            light.xy = vec2(1.0);
       #endif
       ...
       #ifdef COLORRAMP
            diffuseColor.rgb  *= texture2DArray(m_ColorRamp, vec2(light.x, 0.0)).rgb;
            specularColor.rgb *= texture2DArray(m_ColorRamp, vec2(light.y, 0.0)).rgb;
            light.xy = vec2(1.0);
       #endif
